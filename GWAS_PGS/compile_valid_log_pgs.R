...
R20   <- "/gpfs/data/ukb-share/dahl/manuela/pgs_outputs/r2_out_log_log_prediction/"
...
R21   <- "/gpfs/data/ukb-share/dahl/manuela/pgs_outputs/r2_out_default_log_prediction/"
...
for(valid_pop in c("white_euro","afr","asn")){
  out_file <- paste0("/gpfs/data/ukb-share/dahl/manuela/pgs_outputs/final_log_pred/log_vs_lin_",PHENO_LOWER,"_",valid_pop,'_', group, ".txt")
  print(out_file)
  if(file.exists(out_file)) next()
  PGS0_file <- paste0("/gpfs/data/ukb-share/dahl/manuela/pgs_outputs/pgs_out_log/valid_log/",PHENO_LOWER,"_",valid_pop,"_pgs_valid.",thresh0,".sscore")
  PGS1_file <- paste0("/gpfs/data/ukb-share/dahl/manuela/pgs_outputs/pgs_out/valid_log/",PHENO_LOWER,"_",valid_pop,"_pgs_valid.",thresh1,".sscore")
  ...
  prs0 <- merge(pheno_cov,prs0,by="FID")$SCORE1_SUM
  prs1 <- merge(pheno_cov,prs1,by="FID")$SCORE1_SUM
  #shift <- abs(min(pgs_raw)) + 1e-6
  #prs1 <- log(pgs_raw + shift)  # Different from other version! 
  prs.R2 <- r2_diff_log(prs0,prs1,y=log(y),X=X,B=1000)
