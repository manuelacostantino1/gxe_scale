...
if (( $(echo "$lambda == 0" | bc -l) )); then
    gwas_path_male="${GWAS_LOG_MALE}${phenoLower}/"
    gwas_path_female="${GWAS_LOG_FEMALE}${phenoLower}/"
else
    gwas_path_male="${GWAS_MALE}${phenoLower}/"
    gwas_path_female="${GWAS_FEMALE}${phenoLower}/"
fi
mkdir -p "$gwas_path_male"
mkdir -p "$gwas_path_female"

# Covariates file
covar_file="/gpfs/data/ukb-share/extracted_phenotypes/covariates_sa40PC/covariates_sa40PC674178.pheno"
ncovar='3-14'

# Temporary sex-specific keep files in the output dir
keep_females="${gwas_path_female}/keep_females.txt"
keep_males="${gwas_path_male}/keep_males.txt"

awk '$5==2{print $1,$2}' ${keep_file} > ${keep_females}
awk '$5==1{print $1,$2}' ${keep_file} > ${keep_males}

# Output files
out_file_females="${gwas_path_female}chr${chr}_${TRAIN_POP}_${phenoLower}_females"
out_file_males="${gwas_path_male}chr${chr}_${TRAIN_POP}_${phenoLower}_males"

# Run PLINK2 for females
if [[ ! -f "$out_file_females.$pheno_code.glm.linear" ]]; then
    ${LOCAL_PLINK2} --bfile ${pop_path}${pop_file} \
        ...
        --out "$out_file_females"
else
    echo "Output file already exists: $out_file_females.$pheno_code.glm.linear"
fi

# Run PLINK2 for males
if [[ ! -f "$out_file_males.$pheno_code.glm.linear" ]]; then
    ${LOCAL_PLINK2} --bfile ${pop_path}${pop_file} \
        --keep ${keep_males} \
        --pheno "$pheno_file" \
        --pheno-name "$pheno_code" \
    ...
    echo "Output file already exists: $out_file_males.$pheno_code.glm.linear"
fi
