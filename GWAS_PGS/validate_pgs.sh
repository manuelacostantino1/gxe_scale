...
if [[ "$R2_TEST" != "PEARSON" && "$R2_TEST" != "SPEARMAN" ]]; then
    echo "Error: R2_TEST must be either 'PEARSON' or 'SPEARMAN'"
    exit 1
fi

# Take correct input files depending on lambda and test
...
if [ "$R2_TEST" == "PEARSON" ]; then
  ...
  best_thresh="${PGS}best_thresh_spearman/"
  pgs_path="${PGS}valid_spearman/"
fi
